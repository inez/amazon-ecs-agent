...
func (engine *DockerTaskEngine) createContainer(task *apitask.Task, container *apicontainer.Container) dockerapi.DockerContainerMetadata {
	seelog.Infof("Inez's V3 Task engine [%s]: creating container: %s", task.Arn, container.Name)
	...
	hostConfig.Resources.BlkioDeviceReadIOps = blkiodev.ThrottleDevice{"/dev/md0", 28000}  // 1,400,000 / 50
	hostConfig.Resources.BlkioDeviceWriteIOps = blkiodev.ThrottleDevice{"/dev/md0", 13600} // 680,000 / 50
